...
class DirectedGragh{
	...
	void BFS(adjList a){				//任務二的寬度優先BFS走訪相鄰節點 
		...
		while(ptr!=NULL){						 
			...
			adjL[FindPosition(ptr->sid2)].visit = true ;
		...
		while(q.size()!=0 && k < adjL.size()-1 ){		//走訪queue中學號各自的收訊者	
			...
			while(ptr!=NULL && k < adjL.size()-1 ){
				...
				if(!adjL[n].visit&&ptr->sid2!=temp.sid1){		 
...
//				find = true ;
			...
			if(ptr->weigh >= num && adjL[n].visit == false){
				bool have = false ;
				adjL[n].visit = true ;
				for(int i = 0; i<temp.head.size() ; i++){
					if(ptr->sid2 < temp.head[i]){
	...
	vector<adjList> hashL ;
	
	void DoubleHash(adjList doubleHash[SetSize(aList.size()*1.2)]){					//任務二:雙重雜湊 
			...
			if(doubleHash[hash(aList[j].sid1)].hvalue == 0){
				...
				while(true){
					if(doubleHash[key].hvalue == 0){
						doubleHash[key].sid1 = aList[j].sid1 ; 
						doubleHash[key].hvalue = hash(aList[j].sid1) ;
						break ;				
					}
